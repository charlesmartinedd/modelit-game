...
import './WorkspaceLevel9.css'

const WorkspaceLevel9 = ({
  components,
  arrows,
  onRemoveComponent,
  onAddArrow,
  ...
  availableScales = ['Molecular', 'Cellular', 'Tissue', 'Organism', 'Ecosystem', 'Genetic']
}) => {
  ...
  const [activeScale, setActiveScale] = useState(availableScales[0] || 'Cellular')
  const [drawingCrossScale, setDrawingCrossScale] = useState(false)
  const workspaceRef = useRef(null)

  // Get unique scales from components
  const scales = [...new Set(components.map(c => c.scale))].filter(Boolean)
  const displayScales = scales.length > 0 ? scales : availableScales

  // Filter components by active scale
  const activeComponents = components.filter(c => c.scale === activeScale)

  // Get cross-scale arrows (arrows between different scales)
  const crossScaleArrows = arrows.filter(arrow => {
    const fromComp = components.find(c => c.id === arrow.fromId)
    const toComp = components.find(c => c.id === arrow.toId)
    return fromComp && toComp && fromComp.scale !== toComp.scale
  })

  // Get same-scale arrows (arrows within the same scale)
  const sameScaleArrows = arrows.filter(arrow => {
    const fromComp = components.find(c => c.id === arrow.fromId)
    const toComp = components.find(c => c.id === arrow.toId)
  ...
  const handleComponentMouseDown = (component, e) => {
    ...
    if (mode === 'move') {
      ...
      if (drawingArrow && arrowStart && arrowStart.id !== component.id) {
        // Check if this is a cross-scale arrow
        const isCrossScale = arrowStart.scale !== component.scale
  ...
  return (
    <div className="workspace-level9">
      <div className="workspace-header">
        <div className="header-top">
          <h3>Multi-Scale Model</h3>
        ...
        <div className="workspace-instructions">
          {mode === 'move' && <span>💡 Drag components to reposition them on this scale</span>}
          {mode === 'arrow' && !drawingArrow && <span>🎯 Click a component to start drawing an arrow (can connect across scales!)</span>}
          {mode === 'arrow' && drawingArrow && arrowStart && (
            <span className="drawing-mode">
              ✨ Click another component on ANY scale to complete the {arrowType === 'positive' ? 'green (+)' : 'red (-)'} arrow!
              {arrowStart.scale !== activeScale && <span className="cross-scale-hint"> 🌐 Cross-scale connection!</span>}
            </span>
          )}
          {mode === 'state' && <span>⚙️ Click a component to toggle its state (ON/OFF)</span>}
        </div>

        {/* Cross-Scale Connection Summary */}
        {crossScaleArrows.length > 0 && (
          <div className="cross-scale-summary">
            <span className="summary-icon">🌐</span>
            <span className="summary-text">
              {crossScaleArrows.length} cross-scale connection{crossScaleArrows.length !== 1 ? 's' : ''}
            </span>
          </div>
        )}
      </div>
      ...
      >
        <svg className="arrows-layer">
          {sameScaleArrows.map((arrow, index) => {
        ...
        <div className="components-canvas">
          {activeComponents.length === 0 ? (
            <div className="empty-canvas">
              <div className="empty-icon">{getScaleIcon(activeScale)}</div>
              <p>No components on {activeScale} scale yet</p>
              <p className="empty-hint">Add components from the palette below</p>
            </div>
          ) : (
...
export default WorkspaceLevel9
